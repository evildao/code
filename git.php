...
/*
   ...
   | Git 更新器（ZIP版）
   +----------------------------------------------------------------------
   | Copyright (c) 2014 http://www.xiaoliu.org All rights reserved.
   +----------------------------------------------------------------------
...
$config = array(
	'git_password'	=> 'admin',
	'git_zip_url'	=> 'https://github.com/evildao/code/archive/master.zip',
	'zip_path'		=> './git.zip',
	'unZip_path'	=> './',
	'in_zip_path' 	=> 'code-master/'
);


set_time_limit(0);  // 修改为不限制超时时间(默认为30秒)
date_default_timezone_set("PRC");
/* $hook = json_decode($_REQUEST["hook"]);//json转换
if($hook->password != $config['git_password']) {
	exit('ERROR!');
}
 */

file_put_contents($config['zip_path'], sendGET($config['git_zip_url']));

$archive  = new PHPZip();

$zipfile   = $config['zip_path'];
$savepath  =  $config['unZip_path'];
...
FileUtil::moveDir($config['unZip_path'].$config['in_zip_path'], $config['unZip_path']); 
FileUtil::unlinkFile($config['zip_path']);
